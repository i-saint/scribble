...
rpsTCPSocket::rpsTCPSocket()
    : m_socket(INVALID_SOCKET)
    , m_needs_shutdown(true)
{
}

rpsTCPSocket::rpsTCPSocket(SOCKET s, bool needs_shutdown)
    : m_socket(s)
    , m_needs_shutdown(needs_shutdown)
{
}

rpsTCPSocket::rpsTCPSocket(const char *host, uint16_t port)
    : m_socket(INVALID_SOCKET)
    , m_needs_shutdown(true)
{
    open(host, port);
...
SOCKET rpsTCPSocket::getHandle() const { return m_socket; }

bool rpsTCPSocket::open(const char *host, uint16_t port)
{
    close();
...
int rpsTCPSocket::read(void *buf, uint32_t buf_size)
{
    return recv(m_socket, (char*)buf, (int)buf_size, 0);
}

int rpsTCPSocket::write(const void *data, uint32_t data_size)
{
...
bool rpsRunTCPServer(uint16_t port, const std::function<bool (rpsTCPSocket &client)> &handler)
{
    ...
    for(;;) {
        ...
        rpsTCPSocket client(client_sock);
        if(!handler(client)) {
            break;
        }
    }
...
rpsProtocolSocket::rpsProtocolSocket() {}
rpsProtocolSocket::rpsProtocolSocket(SOCKET s, bool needs_shutdown) : m_socket(s, needs_shutdown) {}
rpsProtocolSocket::rpsProtocolSocket(const char *host, uint16_t port) : m_socket(host, port) {}
rpsProtocolSocket::~rpsProtocolSocket() {}
...
bool rpsProtocolSocket::write(const void *data, uint32_t data_size)
{
